...
* A [TI BQ25505 EVM](docs/bq25505_evaluation_board.pdf) is connected to an energy source (e.g. a solar panel) and a battery/supercap (e.g. a LiIon coin cell).
...
* On the [NINA-B1 EVK](docs/EVK-NINA-B1_UserGuide_(UBX-15028120).pdf) R4 and R6 must be removed.
...
* A [u-blox SARA-N2xx EVK](docs/EVK-N2_UserGuide_(UBX-17005115).pdf) is configured so that all of the lower board is off (the lower board is still necessary as the SIM card holder is on the lower board): main power toggle switch is off, all slide switches are off (i.e. on the side towards the middle of the board) and the 3V8 jumper on the lower board (near the SIM) is removed.  Power is then applied directly to the module by connecting `VOR` from the TI BQ25505 EVM to any one of pins 7, 8, 9 or 10 of J101 (ground is pin 1 on both J101 and J104).
...
![hw configuration](images/hw_configuration.jpg "HW configuration")
...
Instead of using a SARA-N2xx EVK, it is possible to use a [SARA-R410M EVK](docs/EVK-R4_UserGuide_(UBX-16029216).pdf), though note that this is NOT yet fully tested:
...
![HW configuration](images/16_hour_run_hw_configuration.jpg "HW configuration")
...
![operating overview](images/16_hour_run_overview.jpg "Operating overview")

Here are some detailed views of the start and end of an "active" period (i.e. when `VBAT_SEC_ON` is low):

![start of active period](images/16_hour_run_start_detail.jpg "Start of active period")
